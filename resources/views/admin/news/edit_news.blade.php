    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('edit.news.store') }}" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    <div class="col-sm-3"><h6 class="mb-0">تصنيف الخبر</h6></div>
                    <div class="col-sm-9 text-secondary">
                        <select class="form-select" name="news_category_id">
                            <option value="" disabled>اختر تصنيف الخبر</option>
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('news_category_id', $value->news_category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">
                    <div class="col-sm-3"><h6 class="mb-0">العرض في شريط عاجل؟</h6></div>
                    <div class="col-sm-9 text-secondary">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="show_in_ticker" name="show_in_ticker" value="1" {{ old('show_in_ticker', $value->show_in_ticker) ? 'checked' : '' }}>
                            <label class="form-check-label" for="show_in_ticker">تفعيل العرض في شريط الأخبار العاجلة</label>
                        </div>
                        @error('show_in_ticker') <div class="text-danger">{{ $message }}</div> @enderror
                    </div>
                </div>
