...
@section('title', 'السوق | ' . $subcategory->name)
@section('main')
...
<main class="max-w-7xl mx-auto px-4 lg:px-8 py-10">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 text-xs md:text-sm text-slate-500 dark:text-slate-400 mb-6 font-medium">
        <div class="flex items-center gap-1.5 flex-wrap">
            ...
            <a href="{{ route('market.public.main', $subcategory->mainCategory->id) }}" class="hover:text-primary transition-colors">
                {{ $subcategory->mainCategory->name }}
            </a>
            <span class="material-symbols-outlined text-xs">chevron_left</span>
            
            <span class="text-slate-400 dark:text-slate-600">{{ $subcategory->name }}</span>
        </div>
    ...
    {{-- Success Banner --}}
    @if(session('success'))
        <div class="mb-6 flex items-center gap-3 bg-green-50 dark:bg-green-950/20 border border-green-200/50 dark:border-green-800/30 text-green-800 dark:text-green-400 rounded-2xl px-5 py-4 shadow-sm animate-pulse-once">
    ...
    <div class="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
        <div class="max-w-3xl">
            <h1 class="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-white leading-tight mb-2">
                {{ $subcategory->name }} للبيع في الكويت <span class="text-slate-400 dark:text-slate-500 text-lg font-bold">({{ $items->count() }})</span>
            </h1>
            <p class="text-xs md:text-sm text-slate-555 dark:text-slate-400 leading-relaxed">
                {{ $subcategory->description ?? 'اكتشف عروض ' . $subcategory->name . ' الجديدة والمستعملة للبيع في الكويت.' }}
            </p>
        </div>
    ...
    <div class="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 mb-10 pb-6 border-b border-slate-200 dark:border-slate-800">
        ...
        <div class="flex-1 overflow-x-auto no-scrollbar flex items-center gap-3 py-1">
            @if($subsubcategories->count() > 0)
                ...
                @endforeach
            @else
                <span class="text-xs text-slate-400 dark:text-slate-650">لا توجد أقسام متفرعة إضافية</span>
            @endif
        </div>
